  ...
  <q-page class="flex justify-center items-center">
    <q-card class="q-card-desktop q-my-xl gt-xs no-shadow">
      <q-item class="card-header q-px-lg q-py-lg">
        ...
        <q-item-section>
          <q-item-label class="text-weight-bold text-body1">تاریخچه پرداخت</q-item-label>
        </q-item-section>
        <div class="flex items-center q-gutter-x-md">
          <q-icon class="dark-3 cursor-pointer" size="md" name="arrow_back" @click="$router.go(-1)">
            <q-tooltip>بازگشت</q-tooltip>
          </q-icon>
        </div>
      </q-item>
      <q-separator />
      <q-linear-progress class="business-progress" indeterminate size="xs" v-if="loadingData" />
      <q-card-section class="q-px-lg q-gutter-y-md">
        ...
        <q-card class="row justify-between items-center no-shadow q-py-md q-px-md cursor-pointer"
          v-for="(item, index) in rows" :key="index" @click="selectCard(index)"
          ...
          <div class="expire-date-container col-3 flex items-center q-gutter-x-xl">
            <label class="dark-2"><q-icon class="expire-date-clock dark-icon2" name="attach_money" size="sm" />
              {{ item.amount }}
              <q-tooltip>مبلغ</q-tooltip>
            </label>
          </div>
          <div class="expire-date-container col-4 flex items-center justify-center q-gutter-x-xl">
            <q-badge v-if="item.statusTitle == 'Enum_BusinessPaymentStatus_Trial'" color="orange" text-color="white"
              label="دوره آزمایشی" class="q-py-xs" />
            <q-badge v-else color="orange" text-color="white" label="پرداخت شده" class="q-py-xs" />
          </div>
          <div class="more-options col-1 q-pl-md">
            <q-btn class="more-icon dark-2" unelevated falt round icon="more_vert" size="md" dense>
...
<script setup>
  ...
  async function loadData(data) {
    ...
    const response = await fetchWrapper
      .post(`business/getBusinessPaymentGridData/${businessId}`, {
