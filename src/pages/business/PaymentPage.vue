<template>
  <q-card class="q-card-desktop gt-xs no-shadow q-my-xl">
    <data-view :dataSource="`business/getBusinessPaymentGridData/${$route.params.businessId}`" orderByField="fromDate"
      searchField="amount" @page-changed="loadData">
      <template #header>
        <q-item class="card-header q-px-lg q-py-lg">
          ...
          <div class="flex items-center q-gutter-x-md">
            <q-icon class="dark-3 cursor-pointer" size="sm" name="arrow_back" @click="$router.go(-1)">
      ...
      <template #item="{ item }">
        <div class="col-5">
          <div class="flex justify-start">
            <label class=""><q-icon class="expire-date-clock dark-2" name="history" size="md" />
              {{
                  "از: " +
                  item.fromDateString +
                  " " +
        ...
        <div class="col-2 flex items-center">
          <label class=""><q-icon class="expire-date-clock dark-2" name="attach_money" size="xs" />
        ...
        <div class="">
          <q-badge color="warning" text-color="white" label="طرح 3" class="q-py-xs q-px-sm" />
        </div>
        <div class="expire-date-container col-2 flex items-center justify-center q-gutter-x-xl">
          <q-badge v-if="item.statusTitle == 'Enum_BusinessPaymentStatus_Trial'" color="orange" text-color="white"
            label="دوره آزمایشی" class="q-py-xs q-px-sm" />
          <q-badge v-else color="positive" text-color="white" label="پرداخت شده" class="q-py-xs q-px-sm" />
        </div>
