<template>
    ...
    <div class="print-hide">
      <table style="width: 100%; border-collapse: collapse;" :style="borderColor_()">
    <thead>
    ...
    <tbody>
      <tr>
        ...
        <td style="padding: 6px 8px;" :style="borderColor_()" v-if="formStore.totalDiscount.value">تخفیف</td>
        <td style="padding: 6px 8px;" :style="borderColor_()" v-if="formStore.totalDiscount.value">مبلغ پس از تخفیف</td>
        <td style="padding: 6px 8px;" :style="borderColor_()">جمع مالیات و عوارض</td>
      ...
      <tr v-for="(item, index) in model.value.invoiceItems" :key="item.id">
        <td style="padding: 6px 8px;" :style="borderColor_()">{{ index + 1 }}</td>
        <td style="padding: 6px 8px;" :style="borderColor_()">{{ item.productCode }} {{ item.productTaxCode }}</td>
        <td style="padding: 6px 8px;" :style="borderColor_()">
          <div class="text-wrap">
            {{ item.productTitle }}
            <small v-if="item.comment">({{ item.comment }})</small>
        ...
        <td style="padding: 6px 8px;" :style="borderColor_()">{{ item.quantity.toLocaleString() }}</td>
        <td style="padding: 6px 8px;" :style="borderColor_()">{{ item.productUnitTitle }}</td>
        <td style="padding: 6px 8px;" :style="borderColor_()">{{ item.price.toLocaleString() }}</td>
        <td style="padding: 6px 8px;" :style="borderColor_()">{{ (item.quantity * item.price).toLocaleString() }}</td>
        <td style="padding: 6px 8px;" :style="borderColor_()" v-if="formStore.totalDiscount.value">
          {{ item.discount.toLocaleString() }}
        </td>
        <td style="padding: 6px 8px;" :style="borderColor_()" v-if="formStore.totalDiscount.value">
          {{ (item.quantity * item.price - item.discount).toLocaleString() }}
        </td>
        <td style="padding: 6px 8px;" :style="borderColor_()">{{ item.vatAmount.toLocaleString() }}</td>
        <td style="padding: 6px 8px;" :style="borderColor_()">
          {{ item.totalPrice.toLocaleString() }}
        </td>
      </tr>
      <tr>
        <td style="padding: 6px 8px;" :style="borderColor_()" colspan="6" class="text-right">
          <strong>جمع کل:</strong>
          ({{ numberToWords(formStore.totalPrice.value) }}
          <b>{{ model.value.currencyTitle }})</b>
        </td>
