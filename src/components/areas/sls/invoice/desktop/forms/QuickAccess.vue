<template>
  <q-card class="bordered fit no-shadow">
    <q-tabs v-model="tab" inline-label indicator-color="primary" outside-arrows class="border-radius-lg text-on-dark">
      <q-tab name="my-products" class="q-py-sm">
      ...
      <q-tab name="test-1" class="q-py-sm">
        <template #default>
      ...
      <q-tab name="test-2" class="q-py-sm">
        <template #default>
          <q-icon name="o_question_mark" size="xs" class="q-mr-sm" />
          <div class="text-body3 text-bold">
            تست 2
    ...
    <q-tab-panels v-model="tab" animated keep-alive class="transparent">
      <q-tab-panel name="my-products" class="no-padding">
        <div class="flex justify-center q-pa-md">
          <q-input color="grey-5" outlined v-model="searchText" placeholder="جستجو در کالا" dense rounded clearable
            clear-icon="o_close" class="full-width text-caption">
        ...
        <q-scroll-area style="height:calc(100vh - 80px);" class="q-mb-md" :thumb-style="helper.thumbStyle"
          :bar-style="helper.barStyle">
          <div class="q-gutter-y-md q-pa-sm q-mx-sm">
            <q-list>
              <q-item v-for="(product, index) in products" :key="index" clickable v-close-popup
                class="q-py-sm border-radius-xs text-on-dark" :class="pulseProduct"
                @click="createInvoice.incrementQuantity(product)">
                <template v-if="createInvoice.getProductQuantity(product.id) > 0">
                  <q-btn unelevated round size="sm" color="secondary" class="z-max text-bold q-ma-sm absolute-top-left"
                    @click.stop="createInvoice.incrementQuantity(product)">
                    <div class="text-body1">
                      {{ createInvoice.getProductQuantity(product.id) }}
                    </div>
                  </q-btn>
                  <q-btn class="absolute-top-right" flat unelevated round color="red" size="sm"
                ...
                <div class="row q-gutter-x-sm items-center q-my-sm">
                  <q-avatar class="border-radius-xs" square v-if="product.picture" color="primary" text-color="white"
                    size="58px">
                    <img :src="product.picture">
                  </q-avatar>

                  <q-avatar class="border-radius-xs" square size="58px" color="primary" text-color="white" v-else>
