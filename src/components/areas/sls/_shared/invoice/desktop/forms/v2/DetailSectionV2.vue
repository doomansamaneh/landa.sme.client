  ...
  <div class="column q-py-md q-gutter-y-md">
    ...
    <div v-for="row in createInvoice.rows.value" :key="row.id">
      <div class="row q-gutter-md">
        ...
        <div style="width: 10%">
          <custom-input v-model="row.price" display-format="n0" />
        </div>
        <div style="width: 15%">
  ...
  <div class="clear-fix row q-my-xl">
    ...
    <div class="col-3 q-gutter-y-md">
      ...
      <div class="row q-mr-md">
        ...
        <div>
          <span class="text-red">({{ totalDiscount.toLocaleString() }})</span>
...
const rowPrice = (row) => Number(row.quantity) * Number(row.price);

const subTotalAmount = computed(() => {
  return createInvoice.rows.value.reduce(
    (total, row) => total + Number(row.quantity) * Number(row.price),
    0
  );
});
...
const rowVat = computed(() => {
  const vatTotal = createInvoice.rows.value.reduce(
