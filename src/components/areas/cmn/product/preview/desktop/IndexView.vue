<template>
  ...
  <div>
    ...
    <q-tab-panels class="bordered rounded-borders q-mt-md" v-model="tab" animated>
      ...
      <q-tab-panel name="history">
        <div style="margin: 24px 56px 0 56px;" class="q-gutter-y-md">
          <div class="text-body1"><q-icon name="o_comment" size="sm" class="icon q-pr-sm" />یادداشت</div>
          <q-editor v-model="editor" placeholder="در اینجا می‌توانید یادداشت بگذارید..." />
        ...
        <div class="q-my-xl q-mx-md">
          <div class="q-pb-md text-body1" style="margin: 0 38px;"><q-icon name="o_history" size="sm"
              class="icon q-pr-sm" />تاریخچه</div>
          <div class="profile-timeline">
            <q-timeline color="accent">
              <q-timeline-entry avatar="https://cdn.quasar.dev/img/avatar4.jpg" v-for="item in items" :key="item">
                <template #default>
                  <q-card class="bordered-1 no-shadow q-mx-xl q-pa-md">
                    <q-card-section class="row justify-between items-center">
                      <div class="row items-center q-gutter-sm">
                        <q-icon :name="item.icon" size="sm" color="accent" class="q-mr-sm" />
                        <div class="text-caption text-bold">{{ item.name }}</div>
                        <div class="row items-center">
                          <span class="text-caption">چند دقیقه پیش</span>
                          <q-tooltip transition-show="scale" transition-hide="scale" anchor="center left"
                            self="center right" :offset="[10, 10]" :delay="700" class="glass text-on-dark text-caption">
                            {{ item.date }}
                          </q-tooltip>
                        </div>
                      </div>
                    ...
                    <q-card-section v-if="item.commentValue">
                      <p class="line-height-sm" contenteditable="true" v-show="!editCommentBtn">
                        {{ item.comment }}
                      </p>
                      <div class="q-gutter-y-md" v-show="editCommentBtn">
                        <q-editor v-model="editCommentValue" />
                        <div class="q-gutter-x-sm">
                          <q-btn @click="editCommentBtn = false" flat rounded class="bg-primary text-white">
                            <q-icon name="o_comment" size="xs" class="q-mr-sm" />
                            <span>ذخیره</span>
                          </q-btn>
                      ...
                      <div v-if="!editCommentBtn" class="row justify-end">
                        <q-btn @click="editComment" unelevated rounded class="text-on-dark">
                          <q-icon name="o_edit" size="xs" class="q-mr-sm" />
                          <span>ویرایش</span>
                        </q-btn>
                        <q-btn @click="deleteAlert" unelevated rounded class="text-on-dark">
