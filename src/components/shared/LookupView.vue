<template>
  <q-input ref="search" outlined required :rules="[(val) => val && val.length > 0]" lazy-rules dense
    class="input lookup" v-model="selectedRow" @update:model-value="searchInLookup" @keydown.down="selectNext"
    @keydown.up="selectPrevious" @keydown.enter.prevent.stop="selectRow" debounce="2000">
    <template #append>
      <q-icon name="clear" size="16px" color="primary" v-if="selectedRow && selectedRow.length > 0"
        class="cursor-pointer" @click="clearSearch" />

      <q-icon @click="lookupShow" name="o_expand_more" id="expand-more-icon" class="show-lookup-icon cursor-pointer"
        size="sm" />
    </template>
    <q-menu fit no-parent-event @show="onMenuShow" @hide="onMenuHide" ref="popup" transition-show="jump-down"
      transition-hide="jump-up" no-refocus no-focus>
      <q-card class="plan-title-card no-shadow">
        <q-inner-loading :showing="loadingData">
          <q-spinner size="50px" color="primary" />
        </q-inner-loading>
        <table id="table" class="plan-title-table text-left full-width text-caption" tabindex="0">
          <thead class="lookup-table-head">
            <tr class="">
              ...
              <th class="" style="width: 50%">
                <q-icon v-if="pagination.sortBy === 'title'" :name="
                    pagination.descending ? 'arrow_drop_up' : 'arrow_drop_down'
                  " size="20px" color="primary" />
                <span @click="sortSelectedColumn('title')" class="cursor-pointer">عنوان</span>
              </th>
              <th class="" style="width: 30%">
                <q-icon v-if="pagination.sortBy === 'payedAmount'" :name="
                    pagination.descending ? 'arrow_drop_up' : 'arrow_drop_down'
                  " size="20px" color="primary" />
                <span class="cursor-pointer" @click="sortSelectedColumn('payedAmount')">هزینه ماهانه</span>
              </th>
            </tr>
          </thead>
          <tbody class="lookup-table-body">
            <tr v-for="(item, index) in rows" :key="item.id" :class="{ 'selected-row': index === selectedCardIndex }"
